...
export default function RecordingInterface() {
  ...
  useEffect(() => {
    setIsClient(true);
    // Dynamic import of react-media-recorder
    import('react-media-recorder').then(({ useReactMediaRecorder }) => {
      // This will be handled in a separate effect
    });
  }, []);

  const supportedFormat = isClient ? getSupportedAudioFormat() : null;
  const recordingSupported = isClient ? isRecordingSupported() : false;

  // Placeholder for media recorder hook - will be replaced with actual implementation
  const status = 'idle';
  const startRecording = () => {};
  const stopRecording = () => {};
  const mediaBlobUrl = null;
  const recorderError = null;
